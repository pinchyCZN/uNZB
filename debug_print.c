...
void print_msg(int msg,int lparam,int wparam)
{
	int i;
	DWORD time;
	time=GetTickCount();
	for(i=0;i<1000;i++){
		...
		if(wm_names[i].val==msg){
			printf("msg=%-20s lparam=%08X wparam=%08X x=%3i y=%3i %08X\n",wm_names[i].name,lparam,wparam,LOWORD(lparam),HIWORD(lparam),time);
			return;
		}
	}
	printf("msg=%08X lparam=%08X wparam=%08X x=%3i y=%3i %08X\n",msg,lparam,wparam,LOWORD(lparam),HIWORD(lparam),time);
}
